...
func (s *SpotifyAPI) create(username string) (string, error) {
	data := fmt.Sprintf("password_repeat=%s&displayname=%s&password=%s&birth_month=%02d&key=142b583129b2df829de3656f9eb484e6&birth_day=%02d&iagree=true&creation_point=client_mobile&platform=Android-ARM&birth_year=%d&gender=neutral&email=%s@onetimeone.my.id",
		s.Password,
		username,
		s.Password,
		rand.Intn(12)+1,
 ...
 ` + "\n")
	...
	for i := 0; i < amount; i++ {
		...
		if createResponse.Status == 1 {
			color.New(color.FgGreen).Println("   > Email : " + username + "@onetimeone.my.id" + " | " + "Password : " + password + " | CREATE ACCOUNT SUCCESS")
			fmt.Fprintf(file, "Email : %s@onetimeone.my.id | Password : %s | CREATE ACCOUNT SUCCESS\n", username, password)
			count++
		} else {
			color.New(color.FgRed).Println("   > Email : " + username + "@onetimeone.my.id" + " | " + "Password : " + password + " | CREATE ACCOUNT FAILED")
		}
	}
}
